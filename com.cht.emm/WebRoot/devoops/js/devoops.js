...
function LoadCalendarScript(callback) {
	function LoadFullCalendarScript() {
		if (!$.fn.fullCalendar) {
			$.getScript(getWebContextPath()
					+ 'devoops/plugins/fullcalendar/fullcalendar.js', callback);
		} else {
	...
	if (!$.fn.moment) {
		$.getScript(getWebContextPath()
				+ 'devoops/plugins/moment/moment.min.js',
...
function LoadTimePickerScript(callback) {
	if (!$.fn.timepicker) {
		$
				.getScript(
						getWebContextPath()
								+ 'devoops/plugins/jquery-ui-timepicker-addon/jquery-ui-timepicker-addon.min.js',
						callback);
	} else {
		if (callback && typeof (callback) === "function") {
			callback();
...
function loadJsTree(callback) {
	if (!$.fn.jstree) {
		$.getScript(getWebContextPath() + '/jstree-master/dist/jstree.js',
				callback);
	} else {
		if (callback && typeof (callback) === "function") {
			callback();
...
function LoadBootstrapValidatorScript(callback) {
	if (!$.fn.bootstrapValidator) {
		$
				.getScript(
						getWebContextPath()
								+ 'devoops/plugins/bootstrapvalidator/bootstrapValidator.js',
						callback);
	} else {
		if (callback && typeof (callback) === "function") {
			callback();
...
function LoadSelect2Script(callback) {
	if (!$.fn.select2) {
		$.getScript(getWebContextPath()
				+ 'devoops/plugins/select2/select2.min.js', callback);
	} else {
		if (callback && typeof (callback) === "function") {
			callback();
		}
...
function LoadFineUploader(callback) {
	if (!$.fn.fineuploader) {
		$
				.getScript(
						getWebContextPath()
								+ 'devoops/plugins/fineuploader/jquery.fineuploader-5.0.1.js',
						callback);
	} else {
		if (callback && typeof (callback) === "function") {
			callback();
...
function LoadXChartScript(callback) {
	function LoadXChart() {
		$.getScript(getWebContextPath()
				+ 'devoops/plugins/xcharts/xcharts.min.js', callback);
	}
	function LoadD3Script() {
		if (!$.fn.d3) {
			$.getScript(
					getWebContextPath() + 'devoops/plugins/d3/d3.v3.min.js',
					LoadXChart);
		} else {
			LoadXChart();
		}
...
function LoadFlotScripts(callback) {
	function LoadFlotScript() {
		$.getScript(
				getWebContextPath() + 'devoops/plugins/flot/jquery.flot.js',
				LoadFlotResizeScript);
	}
	function LoadFlotResizeScript() {
		$.getScript(getWebContextPath()
				+ 'devoops/plugins/flot/jquery.flot.resize.js',
				LoadFlotTimeScript);
	}
	function LoadFlotTimeScript() {
		$.getScript(getWebContextPath()
				+ 'devoops/plugins/flot/jquery.flot.time.js', callback);
	}
	if (!$.fn.flot) {
		LoadFlotScript();
	} else {
...
function LoadMorrisScripts(callback) {
	function LoadMorrisScript() {
		if (!$.fn.Morris) {
			$.getScript(getWebContextPath()
					+ 'devoops/plugins/morris/morris.min.js', callback);
		} else {
			if (callback && typeof (callback) === "function") {
				callback();
			}
	...
	if (!$.fn.raphael) {
		$.getScript(getWebContextPath()
				+ 'devoops/plugins/raphael/raphael-min.js', LoadMorrisScript);
	} else {
		LoadMorrisScript();
	}
}
...
function LoadFancyboxScript(callback) {
	if (!$.fn.fancybox) {
		$.getScript(getWebContextPath()
				+ 'devoops/plugins/fancybox/jquery.fancybox.js', callback);
	} else {
		if (callback && typeof (callback) === "function") {
			callback();
		}
...
function LoadKnobScripts(callback) {
	if (!$.fn.knob) {
		$.getScript(getWebContextPath()
				+ 'devoops/plugins/jQuery-Knob/jquery.knob.js', callback);
	} else {
		if (callback && typeof (callback) === "function") {
			callback();
		}
 ...
 * 
 */
function LoadValidation(callback) {
	$.getScript(getWebContextPath()
			+ 'devoops/plugins/validate/jquery.validate.js', callback);
}

function loadCKeditor(callback) {
	$.getScript(getWebContextPath() + 'resources/js/ckeditor/ckeditor.js',
			callback);
}
...
function LoadSparkLineScript(callback) {
	if (!$.fn.sparkline) {
		$
				.getScript(getWebContextPath()
						+ 'devoops/plugins/sparkline/jquery.sparkline.min.js',
						callback);
	} else {
		if (callback && typeof (callback) === "function") {
			callback();
...
function LoadAjaxContent(url) {
	// http 开头的为外部连接，使用iframe加载，其他为内部连接，加载方式不变
	if (url.indexOf("http") > -1) {
		$("#ajax-content").hide();
		$("#ajax-content-iframe").show();
		$('.preloader').show();
		var height = window.innerHeight - 80;
		// $("#ajax-content").append("<iframe id=\"ajax-content-iframe\"
		// src=\"#\" marginheight=\"0\" marginwidth=\"0\" frameborder=\"0\"
		// scrolling=\"auto\" width=\"100%\" name=\"ajax-content\"
		// ></iframe>");
		$("#ajax-content-iframe").attr("height", height + "px");
		$("#ajax-content-iframe").attr("src", url);
		$("#main").css("padding-right", "0px");
		$("#ajax-content-iframe").attr("onload", function() {
			$('.preloader').hide();
		});
	} else {
		$("#ajax-content-iframe").hide();
		$('#ajax-content').show();
		$("#main").css("padding-right", "15px");
		$('.preloader').show();
		$.ajax({
			mimeType : 'text/html; charset=utf-8', // ! Need set mimeType
			// only
			// when
			// run from local file
			url : url,
					...
					$(this).on(
							...
							function(event) {
								...
								if (table.find("input[name=string-fill]").prop(
										"checked") == true) {
									col.nextAll().find("input[type=text]")
											.each(function() {
												$(this).val($(target).val());
											});
								}
...
function DrawAllCharts() {
	...
	var chart1_data = [
			[ 'Smartphones', 'PC', 'Notebooks', 'Monitors', 'Routers',
					'Switches' ], [ '01.01.2014', 1234, 2342, 344, 232, 131 ],
	...
	var chart2_data = [ [ 'Height', 'Width' ], [ 'Samsung', 74.5 ],
			[ 'Apple', 31.24 ], [ 'LG', 12.10 ], [ 'Huawei', 11.14 ],
			[ 'Sony', 8.3 ], [ 'Nokia', 7.4 ], [ 'Blackberry', 6.8 ],
			[ 'HTC', 6.63 ], [ 'Motorola', 3.5 ], [ 'Other', 43.15 ] ];
	var chart2_options = {
	...
	// Chart 3
	var chart3_data = [ [ 'Age', 'Weight' ], [ 8, 12 ], [ 4, 5.5 ], [ 11, 14 ],
			[ 4, 5 ], [ 3, 3.5 ], [ 6.5, 7 ] ];
	var chart3_options = {
	...
	var chart6_data = [ [ 'Year', 'Sales', 'Expenses' ], [ '2004', 1000, 400 ],
			...
			[ '2007', 1030, 540 ], [ '2008', 2080, 740 ],
			[ '2009', 1949, 690 ], [ '2010', 2334, 820 ] ];
	var chart6_options = {
		backgroundColor : '#fcfcfc',
	...
	// Chart 7
	var chart7_data = [ [ 'Task', 'Hours per Day' ], [ 'Work', 11 ],
...
function TestTable1() {
	$('#datatable-1')
			.dataTable(
					{
						"aaSorting" : [ [ 0, "asc" ] ],
						"sDom" : "<'box-content'<'col-sm-11'f><'col-sm-1 text-right'l><'clearfix'>>rt<'box-content'<'col-sm-6'i><'col-sm-6 text-right'p><'clearfix'>>",
...
function TestTable3() {
	$('#datatable-3')
			.dataTable(
					{
						"aaSorting" : [ [ 0, "asc" ] ],
						"sDom" : "T<'box-content'<'col-sm-11'f><'col-sm-1 text-right'l><'clearfix'>>rt<'box-content'<'col-sm-6'i><'col-sm-6 text-right'p><'clearfix'>>",
						"sPaginationType" : "bootstrap",
						"oLanguage" : {
							"sSearch" : "",
							"sLengthMenu" : '_MENU_'
						...
						"oTableTools" : {
							"sSwfPath" : "devoops/plugins/datatables/copy_csv_xls_pdf.swf",
							"aButtons" : [ "copy", "print", {
								"sExtends" : "collection",
...
function TableXDefaultPageSize(id) {
	var asInitVals = [];
	var oTable = $('#' + id)
			.dataTable(
					{
						"aaSorting" : [ [ 0, "asc" ] ],
						"sDom" : " rt<'box-content'<'col-sm-12'<'tool_box_pageInfo' p>> <'clearfix'>>",
						"sPaginationType" : "bootstrap",
						"oLanguage" : {
							"sSearch" : "",
							"sLengthMenu" : '_MENU_'
	...
	var header_inputs = $("#" + id + " thead input");
	header_inputs.on('keyup', function() {
		/* Filter on the column (the index) of this element */
		oTable.fnFilter(this.value, header_inputs.index(this));
	}).on('focus', function() {
...
function TestTableX(id) {
	...
	var oTable = $('#' + id)
			.dataTable(
					{
						"aaSorting" : [ [ 0, "asc" ] ],
						"sDom" : "<'box-content'<'col-sm-9'><'col-sm-3 div_float_right'f><'clearfix'>>rt<'box-content'<'col-sm-12'<'tool_box_pageInfo' ilp>> <'clearfix'>>",
						"sPaginationType" : "bootstrap",
						"oLanguage" : {
							"sSearch" : "",
							"sLengthMenu" : '_MENU_'
...
function TestTable4() {
	...
	var oTable = $('#datatable-4')
			.dataTable(
					{
						"aaSorting" : [ [ 0, "asc" ] ],
						"sDom" : "<'box-content'<'col-sm-11'f><'col-sm-1 text-right'l><'clearfix'>>rt<'box-content'<'col-sm-6'il><'col-sm-6 text-right'p><'clearfix'>>",
						"sPaginationType" : "bootstrap",
						"oLanguage" : {
							"sSearch" : "",
							"sLengthMenu" : '_MENU_'
...
function TestTable7() {
	...
	var header_inputs = $("#datatable-7 thead input");
	header_inputs.on('keyup', function() {
		/* Filter on the column (the index) of this element */
		oTable.fnFilter(this.value, header_inputs.index(this));
	}).on('focus', function() {
...
function TestTable8() {
	...
	var header_inputs = $("#datatable-8 thead input");
	header_inputs.on('keyup', function() {
		/* Filter on the column (the index) of this element */
		oTable.fnFilter(this.value, header_inputs.index(this));
	}).on('focus', function() {
...
function TestTable9() {
	...
	var header_inputs = $("#datatable-9 thead input");
	header_inputs.on('keyup', function() {
		/* Filter on the column (the index) of this element */
		oTable.fnFilter(this.value, header_inputs.index(this));
	}).on('focus', function() {
...
function TestTableGroup() {
	var asInitVals = [];
	var oTable = $('#datatable-group')
			.dataTable(
					{
						"aaSorting" : [ [ 0, "asc" ] ],
						"sDom" : "<'box-content'<'col-sm-6'f><'col-sm-6 text-right'l><'clearfix'>>rt<'box-content'<'col-sm-6'i><'col-sm-6 text-right'p><'clearfix'>>",
	...
	var header_inputs = $("#datatable-group thead input");
	header_inputs.on('keyup', function() {
		/* Filter on the column (the index) of this element */
		oTable.fnFilter(this.value, header_inputs.index(this));
	}).on('focus', function() {
...
function TestTableUser() {
	var asInitVals = [];
	var oTable = $('datatable-user')
			.dataTable(
					{
						"aaSorting" : [ [ 0, "asc" ] ],
						"sDom" : "<'box-content'<'col-sm-6'f><'col-sm-6 text-right'l><'clearfix'>>rt<'box-content'<'col-sm-6'i><'col-sm-6 text-right'p><'clearfix'>>",
	...
	var header_inputs = $("#datatable-user thead input");
	header_inputs.on('keyup', function() {
		/* Filter on the column (the index) of this element */
		oTable.fnFilter(this.value, header_inputs.index(this));
	}).on('focus', function() {
...
function TestTable10() {
	var asInitVals = [];
	var oTable = $('#datatable-fenpei')
			.dataTable(
					{
						"aaSorting" : [ [ 0, "asc" ] ],
						"sDom" : "<'box-content'<'col-sm-6'f><'col-sm-6 text-right'l><'clearfix'>>rt<'box-content'<'col-sm-6'i><'col-sm-6 text-right'p><'clearfix'>>",
	...
	var header_inputs = $("#datatable-fenpei thead input");
	header_inputs.on('keyup', function() {
		/* Filter on the column (the index) of this element */
		oTable.fnFilter(this.value, header_inputs.index(this));
	}).on('focus', function() {
...
function datatable_safe_policy() {
	var asInitVals = [];
	var oTable = $('#datatable-safe-policy')
			.dataTable(
					{
						"aaSorting" : [ [ 0, "asc" ] ],
						"sDom" : "<'box-content'<'col-sm-6'f><'col-sm-6 text-right'l><'clearfix'>>rt<'box-content'<'col-sm-6'i><'col-sm-6 text-right'p><'clearfix'>>",
	...
	var header_inputs = $("#datatable-safe-policy thead input");
	header_inputs.on('keyup', function() {
		/* Filter on the column (the index) of this element */
		oTable.fnFilter(this.value, header_inputs.index(this));
	}).on('focus', function() {
...
function TestTable11() {
	var asInitVals = [];
	var oTable = $('#datatable-11')
			.dataTable(
					{
						"aaSorting" : [ [ 0, "asc" ] ],
						"sDom" : "<'box-content'<'col-sm-6'f><'col-sm-6 text-right'l><'clearfix'>>rt<'box-content'<'col-sm-6'i><'col-sm-6 text-right'p><'clearfix'>>",
	...
	var header_inputs = $("#datatable-11 thead input");
	header_inputs.on('keyup', function() {
		/* Filter on the column (the index) of this element */
		oTable.fnFilter(this.value, header_inputs.index(this));
	}).on('focus', function() {
...
function TestTable12() {
	var asInitVals = [];
	var oTable = $('#datatable-12')
			.dataTable(
					{
						"aaSorting" : [ [ 0, "asc" ] ],
						"sDom" : "<'box-content'<'col-sm-6'f><'col-sm-6 text-right'l><'clearfix'>>rt<'box-content'<'col-sm-6'i><'col-sm-6 text-right'p><'clearfix'>>",
	...
	var header_inputs = $("#datatable-12 thead input");
	header_inputs.on('keyup', function() {
		/* Filter on the column (the index) of this element */
		oTable.fnFilter(this.value, header_inputs.index(this));
	}).on('focus', function() {
	...
	Morris
			.Line({
				...
				ykeys : [ 'Win8', 'Win7', 'Vista', 'NT', 'XP', 'Linux', 'Mac',
						'Mobile' ],
				labels : [ 'Win8', 'Win7', 'Vista', 'NT', 'XP', 'Linux', 'Mac',
						'Mobile' ]
			});
	Morris.Donut({
...
function DrawSparklineDashboard() {
	...
	var sparkline_clients = [ [ 309 ], [ 223 ], [ 343 ], [ 652 ], [ 455 ],
			[ 18 ], [ 912 ], [ 15 ] ];
	$('.bar').each(function() {
	...
	var sparkline_table = [ [ 1, 341 ], [ 2, 464 ], [ 4, 564 ], [ 5, 235 ],
			[ 6, 335 ], [ 7, 535 ], [ 8, 642 ], [ 9, 342 ], [ 10, 765 ] ];
...
function FileUpload() {
			...
			.on(
					...
					function(event, id, name, response) {
						...
						if (response.success) {
							...
							document.getElementById("version_code").value = response.versionCode;
							document.getElementById("version_name").value = response.versionName;
						}
					});
	 $('#bootstrapped-fine-uploader3')
	.fineUploader({
		template : 'qq-template-bootstrap',
		classes : {
			success : 'alert alert-success',
		...
		callbacks : {
			...
			onComplete:function(id, name, response){
				...
				if (response.success) {
					...
					/**
					 * 现在只处理android的情况，苹果应用以后再处理
					 */
					if(response.pkgName=='nari.mip.console'){
						filePath = response.file;
						fileUrl = response.url;
						pkgName = response.pkgName;
						document.getElementById("version_code").value = response.versionCode;
						...
						alert("请输入包名为 nari.mip.console的平台客户端");
						this.cancel(0);
						deleteUploadFile(response.file);
						//return false;
					}
				}
	...
	/*
	 * $('#uploadIcon').fineUploader({ template : 'qq-template-bootstrap',
	 * classes : { success : 'alert alert-success', fail : 'alert alert-error' },
	 * request : { endpoint : 'upload/receiver/' }, validation : {
	 * allowedExtensions : [ 'jpeg', 'jpg', 'gif', 'png', 'bmp' ], itemLimit : 1 },
	 * callbacks : { onCancel : function(id, fileName) { $.ajax({ url :
	 * "rest/app/cancel", type : "POST", async : true, data : { path : iconPath },
	 * success : function(p_resultValue) { }, error : function(p_request,
	 * p_status, p_err) { } });
	 * 
	 * iconPath = ""; iconUrl = ""; } } }).on('complete', function(event, id,
	 * name, response) {
	 * 
	 * if (response.success) { iconPath = response.file; iconUrl = response.url; }
	 * }); $('#uploadScreenshot').fineUploader({ template :
	 * 'qq-template-bootstrap_1', classes : { success : 'alert alert-success',
	 * fail : 'alert alert-error' }, request : { endpoint : 'upload/receiver/' },
	 * validation : { allowedExtensions : [ 'jpeg', 'jpg', 'gif', 'png', 'bmp' ],
	 * itemLimit : 5 }, callbacks : { onCancel : function(id, fileName) {
	 * $.ajax({ url : "rest/app/cancel", type : "POST", async : true, data : {
	 * path : pathValues[id] }, success : function(p_resultValue) { for ( var i =
	 * 0; i < keys.length; i++) { if (keys[i] == id) { keys.splice(i, 1); break; } }
	 * pathValues[id] = null; urlValues[id] = null; }, error :
	 * function(p_request, p_status, p_err) { } });
	 *  } } }).on('complete', function(event, id, name, response) { if
	 * (response.success) { if (pathValues[id] == null) { keys.push(id); }
	 * pathValues[id] = response.file; urlValues[id] = response.url;
	 * changeScreenshot = true; } }); $('#uploadNewsPic').fineUploader({
	 * template : 'qq-template-bootstrap_2', classes : { success : 'alert
	 * alert-success', fail : 'alert alert-error' }, request : { endpoint :
	 * 'upload/receiver/' }, validation : { allowedExtensions : [ 'jpeg', 'jpg',
	 * 'gif', 'png', 'bmp' ], itemLimit : 3 }, callbacks : { onCancel :
	 * function(id, fileName) { $.ajax({ url : "rest/app/cancel", type : "POST",
	 * async : true, data : { path : pathValues[id] }, success :
	 * function(p_resultValue) { for ( var i = 0; i < keys.length; i++) { if
	 * (keys[i] == id) { keys.splice(i, 1); break; } } pathValues[id] = null;
	 * urlValues[id] = null; }, error : function(p_request, p_status, p_err) { }
	 * }); } } }).on('complete', function(event, id, name, response) {
	 * 
	 * if (response.success) { if (pathValues[id] == null) { keys.push(id); }
	 * pathValues[id] = response.file; urlValues[id] = response.url;
...
function getWebContextPath() {
	...
	if (tmp.length >= 4) {
		...
		return path.substring(0, tmp[3] + 1);
	}
		...
		.ready(
				function() {
					...
					$('.show-sidebar').on('click', function() {
						...
						$('div#top_nav').toggleClass('sidebar-show');
						setTimeout(MessagesMenuWidth, 200);
					});
					$('#left_side_bar').on(
							'click',
							function() {
					...
					if (ajax_url == null || ajax_url.length < 1) {
						// console.log($("#sidebar-left").find("a.ajax-link"));
						ajax_url = $("#sidebar-left").find("a.ajax-link")
								.first().attr("href");
					}
					if (ajax_url != null && ajax_url.length > 0) {
					...
					$('.main-menu')
							.on(
									...
									function(e) {
										...
										// 去除 active标签
										another_items.find('a').removeClass(
												'active');
										another_items.find('a').removeClass(
												'selected');
										another_items.find('a').removeClass(
												'active-parent');
										another_items.find('i.fa-angle-up')
...
function TestTable_alldevice() {
	var asInitVals = [];
	var oTable = $('#datatable-alldevice')
			.dataTable(
					{
						"aaSorting" : [ [ 0, "asc" ] ],
						"sDom" : "<'box-content'<'col-sm-6'f><'col-sm-6 text-right'l><'clearfix'>>rt<'box-content'<'col-sm-6'i><'col-sm-6 text-right'p><'clearfix'>>",
	...
	var header_inputs = $("#datatable-alldevice thead input");
	header_inputs.on('keyup', function() {
		/* Filter on the column (the index) of this element */
		oTable.fnFilter(this.value, header_inputs.index(this));
	}).on('focus', function() {
