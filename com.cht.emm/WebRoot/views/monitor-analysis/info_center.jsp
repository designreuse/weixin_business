				...
				<div class="row">
					<div class="box col-sm-8 col-xs-12"
						...
						<div class="box-header"
							style="background:#FCFCFC;border-bottom:0px">
							<div class="box-name">
								...
								<div class="row">
									<div class="col-xs-6" style="padding:45px 0px;">
										<div style="float:right;">
											<div class="dash_labels_2"
												style="text-align:right;width:100%">过去一天</div>
											<div class="dash_value_big" id="last_day">${day }</div>
										</div>
									</div>
									<div class="col-xs-6"
										...
										<div class="row"
											...
											<p class="dash_value_middle" id="last_week">${week }</p>
										</div>
										<div class="row"
											style="height:100px;padding: 10px 0;font-weight: 200;">
											<p class="dash_labels_2">最近一个月</p>
											<p class="dash_value_middle" id="last_month">${month }</p>
										</div>
									</div>
								</div>
							</div>
...
<script type="text/javascript">
	var source_type = "monitor_analysis";
	...
	function DeviceCountView() {
		...
		$
				.getJSON(
						"rest/" + source_type + "/device/brand",
						function(result) {
							var counts = result.resultValue;
							var datas = [];
							//console.log(counts[0].count);
							if (counts != null && counts[0].count != 0) {
	...
	function top10AppView() {
		...
		$.ajaxSettings.async = false;
		$.getJSON("rest/" + source_type + "/app/tops", function(result) {
			var counts = result.resultValue;
	...
	function DeviceOpsView() {
		...
		$.ajaxSettings.async = false;
		var total = 0;
		$.getJSON("rest/" + source_type + "/device/ops", function(result) {
			var counts = result.resultValue;
			var datas = [];
			if (counts != null && counts.length != 0) {
				for (i = 0; i < counts.length; i++) {
	...
	function DeviceLegalView() {
		...
		var total = 0;
		$.ajaxSettings.async = false;
		$.getJSON("rest/" + source_type + "/device/count", function(result) {
			var counts = result.resultValue;
			var datas = [];
			if (counts != null && counts.length > 0) {
				for (i = 0; i < counts.length; i++) {
	...
	function DeviceAlarmView() {
		...
		var total = 0;
		$.ajaxSettings.async = false;
		$.getJSON("rest/" + source_type + "/device/alarm", function(result) {
			var counts = result.resultValue;
			var datas = [];
			if (counts != null) {
				for (i = 0; i < counts.length; i++) {
	...
	function render() {
		$.getJSON("rest/" + source_type + "/app/download", function(result) {
			if (result.successful) {
				map = result.resultValue;
				$("#last_week").html(map.week);
				$("#last_day").html(map.day);
				$("#last_month").html(map.month);
			}
		});
	}
	...
	function DeviceOnlineView() {
		...
		var total = 0;
		$.ajaxSettings.async = false;
		$.getJSON("rest/" + source_type + "/device/online", function(result) {
			var counts = result.resultValue;
